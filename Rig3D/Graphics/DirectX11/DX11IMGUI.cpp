...
DX11IMGUI::GUIObserver DX11IMGUI::mGUIObserver;
...
void RenderDrawLists(ImDrawData* draw_data)
{
	...
	const D3D11_RECT r = { 0, 0, (LONG)ImGui::GetIO().DisplaySize.x, (LONG)ImGui::GetIO().DisplaySize.y };
	g_pd3dDeviceContext->RSSetScissorRects(1, &r);
}

DX11IMGUI::GUIObserver::GUIObserver() {
	auto mEventHandler = &WMEventHandler::SharedInstance();
	...
	mEventHandler->RegisterObserver(WM_MOUSEWHEEL, this);
	...
	mEventHandler->RegisterObserver(WM_CHAR, this);
}

void DX11IMGUI::GUIObserver::HandleEvent(const IEvent& iEvent)
{
	...
	case WM_MBUTTONUP:
		...
		return;
	case WM_MOUSEWHEEL:
		io.MouseWheel += GET_WHEEL_DELTA_WPARAM(wmEvent.wparam) > 0 ? +1.0f : -1.0f;
		return;
	case WM_MOUSEMOVE:
		io.MousePos.x = (signed short)(wmEvent.lparam);
		io.MousePos.y = (signed short)(wmEvent.lparam >> 16);
		return;
	case WM_KEYDOWN:
		if (wmEvent.wparam < 256)
			io.KeysDown[wmEvent.wparam] = 1;
		return;
	case WM_KEYUP:
		if (wmEvent.wparam < 256)
			io.KeysDown[wmEvent.wparam] = 0;
		return;
	case WM_CHAR:
		// You can also use ToAscii()+GetKeyboardState() to retrieve characters.
		if (wmEvent.wparam > 0 && wmEvent.wparam < 0x10000)
			io.AddInputCharacter((unsigned short)wmEvent.wparam);
		return;
	}
	return;
}
...
bool DX11IMGUI::CreateDeviceObjects()
{
	if (!g_pd3dDevice)
		return false;
	if (g_pFontSampler)
...
void DX11IMGUI::InvalidateDeviceObjects()
{
	if (!g_pd3dDevice)
		return;
...
bool DX11IMGUI::Init(void* hwnd, ID3D11Device* device, ID3D11DeviceContext* device_context)
{
	g_hWnd = (HWND)hwnd;
	g_pd3dDevice = device;
