...
void UIManager::RenderSkillButton(Button* b, vec2f pos)
{
	...
	if (b->skill->mCost) mSpriteManager->DrawTextSprite(SPRITESHEET_FONT_NORMAL, 25, pos + vec2f(35, -47), vec4f(0, 0.51f, 0.75f, 1), ALIGN_CENTER, "%.0f", b->skill->mCost);
	if (b->keySpriteID != -1) mSpriteManager->DrawSprite(SPRITESHEET_CONTROL_ICONS, b->keySpriteID, pos + vec2f(-35, -37), vec2f(28, 28));
...
void UIManager::RenderManaBar()
{
	...
	for each (Ghost& ghost in Factory<Ghost>()) {
		mSpriteManager->DrawSprite(SPRITESHEET_BARS, 3, mSpriteManager->perc2f(0.5f, posYPerc), vec2f(410, 56), vec4f(1, 1, 1, 1), vec2f(ghost.GetManaPerc(), 1));
...
void UIManager::RenderObjectives(GameState gameState, bool isServer)
{
	...
	mSpriteManager->DrawSprite(SPRITESHEET_BARS, 4, anchor + vec2f(50, 6), vec2f(-250, 38), vec4f(0.84f, 1, 0.4f, 1));
