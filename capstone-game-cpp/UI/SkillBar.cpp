...
void SkillBar::RenderObjectives(GameState gameState, bool isServer)
{
	...
	mSpriteManager->DrawTextSprite(SPRITESHEET_FONT_NORMAL, 20, mSpriteManager->perc2f(0, posYPerc) + vec2f(10, 0), vec4f(0.5f, 1, 0.5f, 1), ALIGN_LEFT, "%s", isServer ? "Areas Exorcised" : "Objectives");
	for each (DominationPoint& dp in Factory<DominationPoint>()) {
		if ((dp.mTier == 0 && gameState == GAME_STATE_CAPTURE_0) || (dp.mTier == 1 && gameState == GAME_STATE_CAPTURE_1)) {
			mSpriteManager->DrawTextSprite(SPRITESHEET_FONT_NORMAL, 18, mSpriteManager->perc2f(0, posYPerc) + vec2f(10, 30.0f * ++i + 10), vec4f(1, 1, 1, 1), ALIGN_LEFT, dp.mName);
		}
	}
